...
const TableDatos = () => {
  ...
  const tableA = () => {
    ...
    for (let i = 0; i < numClases; i++) {
      ...
      i === 0
        ? limInf.push(Number(arrDatos[0]))
        : (!Number.isInteger(auxli)
            ? limInf.push(Number(auxli).toFixed(3))
            : limInf.push(auxli),
          (auxli += Number(amplitud)));
    }

    let element;
    let auxls = Number.isInteger(amplitud)
      ? 1
      : amplitud === 0.1
      ? 0.01
      : 0.001;
    for (let i = 0; i < numClases; i++) {
      ...
      limSup.push(
        Number.isInteger(element) ? element : Number(element).toFixed(3)
      );
    }
    ...
    for (let i = 0; i < limInf.length; i++) {
      let marca =
        (Number.parseFloat(limInf[i]) + Number.parseFloat(limSup[i])) / 2;
    ...
    aux = arrDatos.length;
    frecAcum.map((dato, i) => freCom.push(Number(aux) - Number(dato)));
    for (let i = 0; i < numClases; i++) {
      Abc.push(arrAbc[i]);
    }
    setShowTable(true);
  ...
  for (let index = 0; index < users.length; index++) {
    datos[index] = users[index].nivel_agua;
  }
  ...
  return (
    ...
    >
      <div style={{}}>
        <button onClick={datosNTabla} className="btn btn-success">
          Calcular Datos de Nivel de Agua
        </button>
      </div>
      {showData && (
        <div>
          ...
          <h2>
            <b>Numero de datos : </b> {numDatos}
          </h2>
          <h2>
            <b>Amplitud : </b> {amplitud}
          </h2>

          <button onClick={tableA} type="submit" className="btn btn-success">
        ...
        {showTable && (
          <div>
            ...
            <div style={{}}>
              ...
              <h4>
                <b>Desviacion Media:</b> {DVM}
              </h4>
              <h4>
                <b>Varianza:</b> {varianzaF}
              </h4>
              <h4>
                <b>Desviacion Estandar:</b>{" "}
                {Math.pow(varianzaF, 1 / 2).toFixed(3)}{" "}
              </h4>
